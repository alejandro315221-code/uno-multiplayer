...
- **Game modes:**
  ...
  - TwentyOne Cards with optional tokens
  - River Cards
  - Bingo with Easy and Hard draw modes
  - Left Center Right
  - Number Wheel with play-money token picks
  - DiceNumbers with table-style dice rules
  - Two Hand Draw with automated draw rules
  - Chat Room mode
- **CPU players** for filling empty seats.
- **Room chat** with optional profanity filtering and host chat clearing.
- **Family/school-friendly default:** table games stay available, but picking tokens are hidden unless the host turns on the opt-in points switch.
- **Music/SFX controls** and a launch screen that captures a user click so browsers can allow startup audio.
- **No real-money transaction logic:** table-style games use only local virtual token balances and never connect to payment, card-processing, cryptocurrency, or cash-out systems.
...
| Ace | Smooth, flirtatious smooth card-table player |
