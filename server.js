...
//  Then host on Railway / Render / Replit etc.
// ============================================================
...
function nextTurn(room, steps=1) {
  ...
  room.turnIdx = ((room.turnIdx + room.dir * steps) % n + n) % n;
  room.hasDrawn = false;
}
...
function handlePlay(room, playerIdx, cardIdx, chosenColor) {
  ...
  if (player.hand.length === 1) {
    sendChat(room, player.name, 'UNO! 🟡');
  }

  const n = room.players.length;
  if (newVal === 'S') {
    nextTurn(room, 2); // skip next
  } else if (newVal === 'R') {
    room.dir *= -1;
...
function startGame(room) {
  ...
  room.hasDrawn = false;
  room.winner   = null;
  room.players.forEach(p => { p.hand = draw(room, HAND_SIZE); });
  // First card — no wild/special
  let startCard;
  ...
  ws.on('message', (raw) => {
    ...
    if (msg.type === 'join') {
      ...
      if (!code || code.length < 2) {
        ws.send(JSON.stringify({ type:'error', msg:'Invalid room code.' })); return;
      }
      if (!rooms[code]) rooms[code] = makeRoom(code);
      const room = rooms[code];
      if (room.state !== 'lobby' && !msg.rejoin) {
        ws.send(JSON.stringify({ type:'error', msg:'Game already in progress.' })); return;
      }
      if (room.players.length >= MAX_PLAYERS && !msg.rejoin) {
        ws.send(JSON.stringify({ type:'error', msg:'Room is full (max '+MAX_PLAYERS+').' })); return;
      }
      // Check rejoin
      let existingIdx = room.players.findIndex(p => p.name === name);
      if (existingIdx >= 0) {
      ...
      sendChat(room, 'Server', `${name} joined the room.`);
      // Send lobby state
      broadcast(room, { type:'lobby', players: room.players.map(p=>p.name), hostName: room.players[0].name });
      return;
    }

    if (!myRoom) { ws.send(JSON.stringify({ type:'error', msg:'Join a room first.' })); return; }
    const room = myRoom;

    if (msg.type === 'start') {
      if (myIdx !== 0) { ws.send(JSON.stringify({ type:'error', msg:'Only the host can start.' })); return; }
    ...
    if (msg.type === 'chat')  {
      sendChat(room, room.players[myIdx].name, (msg.text||'').slice(0,200));
      return;
    }
    if (msg.type === 'restart') {
      if (myIdx !== 0) return;
...
httpServer.listen(PORT, () => console.log(`UNO Server running on http://localhost:${PORT}`));
